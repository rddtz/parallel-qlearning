...
\begin{algorithmic}[1]
\Require $\text{Grid}(L,C)$, Obstáculos$(O)$, Episódios$(E)$, Max\_Passos$(P)$, $\alpha$, $\gamma$, $\varepsilon$
\State Inicializar $Q[s][a] \leftarrow 0$ para todos os estados $s$ e ações $a$
\State Gerar Grid com $s_0 = (0,0)$, $g = (L{-}1, C{-}1)$ e $O$ obstáculos aleatórios
\For{cada episódio $e = 1, \ldots, E$}
  \State $s \leftarrow s_0$
  \For{cada passo $t = 1, \ldots, P$}
    \If{$\text{Random}(0,1) < \varepsilon$}
      \State $a \leftarrow$ ação aleatória \Comment{exploração}
    \Else
      \State $a \leftarrow \arg\max_{a'} Q[s][a']$ \Comment{explotação}
    \EndIf
    \State $s' \leftarrow \text{próximo\_estado}(s, a)$
    \State $r \leftarrow \text{recompensa}(s')$
    ...
    \State $s \leftarrow s'$
    \If{$s = g$} \textbf{break} \EndIf
  \EndFor
\EndFor
\State $\pi(s) \leftarrow \arg\max_a\, Q[s][a]$ para todo $s$
\end{algorithmic}
\end{algorithm}
#+end_export
