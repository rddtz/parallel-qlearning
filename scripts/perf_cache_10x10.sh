#!/bin/bash
# perf_cache_10x10.sh -- cache miss probe for 10x10 grid across thread counts
#
...
# Fixed parameters matching the experiment
GRID=10
EPISODES=10000
OBSTACLES=0
MAX_STEPS=1000
EPSILON=0.3
SEED=54   # matches run_project.sh default

THREADS_LIST=(1 2 4 8 16 20 40)
SCHEDULES=(static dynamic guided)

# All 5 sync configs from the experiment: "label:mode:interval"
# interval=0 means no --sync-interval flag (auto heuristic).
SYNC_CONFIGS=(
    "sqrt:sqrt:0"
    "every10:sqrt:10"
    "every1000:sqrt:1000"
...
OUT_DIR="$REPO_ROOT/results/perf_cache_10x10_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$OUT_DIR"
